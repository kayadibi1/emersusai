...
// Stacked bar per week, muscles are the stack layers. `weeks[i].by_muscle`
// is a map muscle → sets. Consistent muscle order ensures stacking stays
// readable across weeks.
...
export function WeeklyVolumeTrendProgress({ title, display_width, summary, follow_up_chips, data }) {
  ...
  const totals = weeks.map((w) => muscle_order.reduce((s, m) => s + (w.by_muscle[m] || 0), 0));
