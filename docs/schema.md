...
- **`exercises`** — canonical exercise catalog (69 seeded). Categories: `resistance`, `bodyweight`, `cardio`, `swimming`, `climbing`, `hybrid`. `auto_created=true` flag marks entries created by the fuzzy-match fallback for manual review. Public-readable; only `service_role` can mutate directly (regular users mutate via `resolve_exercise_id` SECURITY DEFINER helper).
