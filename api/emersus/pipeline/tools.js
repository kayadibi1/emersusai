...
const EMIT_CALCULATOR_WIDGET = {
  ...
  strict: true,
  description: [
    "Emit a macro-split donut widget. Use ONLY for daily macro breakdowns — grams/kcal of protein, carbs, fat, optionally compared to TDEE.",
    "",
    "ALWAYS write 2-4 sentences of prose FIRST, then call this tool.",
    "",
    "Data-only: you provide a structured JSON payload, the client renders the donut + legend. No HTML, no CSS. Widget appears inline in the chat.",
    "",
    "CALL WHEN the user asks for a macro breakdown / macro split / daily macros / 'how should I split my calories' / maintenance-calorie-vs-macro-distribution view.",
    "",
    "DO NOT CALL for:",
    "  - 1RM estimates, RPE-to-%1RM conversions, plate loading — use emit_widget (raw HTML) or answer in prose",
    "  - standalone TDEE calculations without a macro breakdown — prose or emit_widget",
    "  - body-fat estimates, pace-per-km, carb cycling schedules — prose or emit_widget",
    "  - protein timing or meal-by-meal protein distribution — prose for now (emit_nutrition_widget lands in Plan 4)",
    "  If the user's request is not a daily macro breakdown, use a different tool or plain prose.",
    "",
    "TEMPLATE SELECTION (the `type` enum has exactly one value in Plan 1):",
    ...
    "- display_width: 'narrow' for a simple donut; 'medium' or 'wide' when you also show TDEE context.",
    "- summary: one-sentence takeaway (e.g., '~400 kcal deficit · 0.4 kg/wk projected loss').",
    "- follow_up_chips: 1-4 short CTAs (e.g., 'Apply to meal plan', 'Swap to higher fat').",
  ].join("\n"),
  parameters: {
    type: "object",
