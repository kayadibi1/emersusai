...
const EMIT_CALCULATOR_WIDGET = {
  ...
  description: [
    ...
    "    Use for ANY 'what should my macros look like' / 'macro split for X kcal' question.",
    "  tdee_calculator — BMR + TDEE card from weight/height/age/sex/activity.",
    "  one_rm_estimator — one-rep-max card from a working set (load × reps).",
    ...
    "",
    "IMPORTANT — SERVER-COMPUTED FIELDS (do NOT invent these; null is fine):",
    "  tdee_calculator: bmr + tdee are computed by the renderer via Mifflin-St Jeor from weight/height/age/sex/activity. Always pass null; the user will see the correct values.",
    "  one_rm_estimator: epley_1rm + brzycki_1rm are computed by the renderer from load + reps. Always pass null.",
    "  body_fat_estimator: body_fat_pct is computed by the renderer via the Navy formula. Always pass null.",
    "  macro_ring: if you only know kcal_total + phase + protein.grams, set carbs/fat legs to null and the renderer will compute a phase-appropriate split. If the user gave a specific split, pass grams+kcal+target_grams through.",
    "  pace_calculator: pace_sec_per_km + speed_kmh are renderer-computed from distance_km + time_sec. Pass null.",
    "  protein_target_calculator: per_meal_g + leucine_threshold_g are renderer-computed; pass null.",
    "",
    "DO NOT CALL for:",
    "  - Protein timing / per-meal macros (per-meal, not daily split) — use emit_nutrition_widget.",
    "",
    "DATA SHAPE (strict: fill the atomic inputs your `type` needs, null everything else):",
    "  macro_ring fills: kcal_total, phase, protein.{grams}. carbs/fat legs optional.",
    "  tdee_calculator fills: weight_kg, height_cm, age, sex, activity_level. bmr/tdee → null.",
    "  one_rm_estimator fills: lift, unit, load, reps. epley_1rm/brzycki_1rm → null.",
    "  body_fat_estimator fills: sex, neck_cm, waist_cm, height_cm (+ hip_cm if female). body_fat_pct → null.",
    "",
    "EXAMPLE macro_ring (user said: '2200 kcal cut with 180g protein'):",
    '  data: { "kcal_total": 2200, "phase": "cut", "protein": { "grams": 180, "target_grams": 180, "kcal": 720 }, "carbs": null, "fat": null, "tdee_reference": null, ...other fields null }',
    "EXAMPLE one_rm_estimator:",
    '  data: { "lift": "Back Squat", "unit": "kg", "load": 100, "reps": 5, "epley_1rm": null, "brzycki_1rm": null, ...other fields null }',
    "EXAMPLE tdee_calculator:",
    '  data: { "weight_kg": 80, "height_cm": 180, "age": 32, "sex": "male", "activity_level": "moderate", "bmr": null, "tdee": null, ...other fields null }',
  ].join("\n"),
  parameters: {
    type: "object",
  ...
  parameters: {
    ...
    properties: {
      ...
      type: { type: "string", enum: [
        ...
        "lift_progress_grid", "weekly_volume_trend",
...
const EMIT_EVIDENCE_WIDGET = {
  ...
  strict: true,
  description: [
    "Emit a QUANTITATIVE evidence visualization. Call ONLY when you have at least 3 studies with non-null quantitative data (n, effect_size, CIs, doses, years) from the retrieved sources. Write 2-4 sentences of prose FIRST, then call.",
    "",
    "DO NOT CALL for (use the family-specific tool or prose instead):",
    "  - 'How should I structure / plan / program X' → emit_training_widget",
    "  - 'When should I take / how do I dose / loading vs maintenance' → emit_pharma_widget",
    "  - 'How should I spread / distribute / split my meals' → emit_nutrition_widget",
    "  - 'Is X supported / is the evidence strong for X' when you have no quantitative data → evidence_strength_card (if you can grade) or PROSE, not study_matrix",
    "  - General 'what does the evidence say' with no numbers retrieved → PROSE, not study_matrix",
    "",
    "TRIGGER PHRASES (only fire when the user wants data, not protocol):",
    "  forest plot, effect sizes, confidence intervals, meta-regression,",
    "  compare the effect sizes, rank the studies by effect, quality of the trials,",
    "  citation timeline, study by study breakdown with the numbers",
    "",
    "TEMPLATE SELECTION:",
    "  study_matrix — table of studies with design, n, effect size, direction.",
    "    REQUIRED: at least 3 rows with BOTH non-null n AND non-null effect_size. If you can't supply these, DO NOT use study_matrix — use evidence_strength_card or write prose.",
    "  effect_size_forest — simple forest plot of effect sizes with 95% CIs. REQUIRED: rows with effect + ci_low + ci_high.",
    "  forest_plot — classic meta-analysis forest with per-study square sized by n, CI whiskers, pooled diamond at bottom. REQUIRED: studies with n + ci bounds + pooled estimate.",
    "  evidence_strength_card — GRADE-style badge card for a claim. Use when you have a qualitative confidence level but lack per-study numbers.",
    ...
    "  ci_ladder — protocols ranked by effect with CI whiskers.",
    "  citation_timeline — studies by year with dot size/opacity tracking citation count.",
    "  study_beeswarm — every study as a dot jittered vertically.",
    "",
    "GROUNDING RULE: Every `citation` string MUST match a real source from the retrieval context. Do not invent author-year labels. Every `n` and `effect_size` MUST come from a retrieved study's text — do not set them to null just to satisfy the schema; if you don't have the number, drop the row entirely.",
    "",
    "DATA SHAPE (strict: fill the fields your `type` uses, null the others):",
    "  study_matrix fills: question, studies (each with non-null n + effect_size)",
    "  effect_size_forest fills: outcome, rows (each with effect + ci_low + ci_high)",
    "  evidence_strength_card fills: claim, level, factors",
    "",
    "EXAMPLE study_matrix (only when numbers are in retrieval):",
