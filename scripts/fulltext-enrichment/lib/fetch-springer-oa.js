...
const limiter = getRateLimiter('springer', { rps: 1.5 }); // 100/min = 1.67 rps
...
export async function fetchForDoi(doi) {
  ...
  try {
    resp = await fetchWithRetry(
      ...
      { label: `springer:${doi}`, maxRetries: 2, baseMs: 2000 }
    );
  } catch (err) {
    // Springer 429 = daily quota hit (per their docs, not rate-limit). Pin and skip.
    if (err.transient && err.status === 429) {
      writeQuota(DAILY_CAP);
      return null;
    }
  ...
  let xml;
  try { xml = await resp.text(); } catch { return null; }
  writeQuota(used + 1);
