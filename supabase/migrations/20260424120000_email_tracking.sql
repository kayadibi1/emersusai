...
create unique index if not exists email_events_dedup_idx
  on public.email_events (resend_id, kind, occurred_at);

create index if not exists email_events_send_occurred_idx
  on public.email_events (send_id, occurred_at desc);
...
create policy email_sends_self_select on public.email_sends
  for select to authenticated using (auth.uid() = user_id);

create policy email_unsubscribes_self_select on public.email_unsubscribes
  for select to authenticated using (auth.uid() = user_id);
